...
void SparseMatrix::HashTable::insert(Element* el)
{
  ...
  if (capacity == size+1)
    throw KII_invalid_argument("SparseMatrix hash table full");
  ...
  while ((table[loc] != &deleted) && (table[loc] != NULL)) {
    if (*table[loc] == *el)
      throw KII_invalid_argument("Attempt to insert duplicate Element into SparseMatrix hash table [1]");
    loc = (loc+1)%capacity;
    if (loc == start)
      throw KII_invalid_argument("Hashtable insert wraparound: unable to insert Element [1]");
  }
  ...
  if (table[loc] == &deleted) {
    ...
    while (table[loc2] != NULL) {
      if (loc2 == start)
	throw KII_invalid_argument("Hashtable insert wraparound: unable to insert Element [2]");
      loc2 = (loc2+1)%capacity;
      if (*table[loc2] == *el)
	throw KII_invalid_argument("Attempt to insert duplicate Element into SparseMatrix hash table [2]");
    }
  }
...
void SparseMatrix::HashTable::update(int r, int c, FLOAT v)
{
  ...
  if (el == NULL)
    throw KII_invalid_argument("Attempt to update Element not in SparseMatrix hash table");
  el->value = v;
}
