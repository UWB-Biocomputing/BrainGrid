...
// Default constructor
AllLIFNeurons::AllLIFNeurons() : AllNeurons()
{
    C1 = NULL;
    C2 = NULL;
    Cm = NULL;
...
void AllLIFNeurons::setupNeurons(SimulationInfo *sim_info)
{
...
void AllLIFNeurons::cleanupNeurons()
{
    if (size != 0) {
        delete[] C1;
        delete[] C2;
 ...
 */
int AllLIFNeurons::numParameters()
{
    return 8;
}
 ...
 */
int AllLIFNeurons::readParameters(const TiXmlElement& element)
{
    int read_params = 0;
 ...
 */
void AllLIFNeurons::printParameters(ostream &output) const
{
 ...
 */
void AllLIFNeurons::createAllNeurons(SimulationInfo *sim_info)
{
 ...
 */
void AllLIFNeurons::setNeuronDefaults(const int index)
{
    Cm[index] = DEFAULT_Cm;
    Rm[index] = DEFAULT_Rm;
 ...
 */
void AllLIFNeurons::initNeuronConstsFromParamValues(int neuron_index, const BGFLOAT deltaT)
{
 ...
 */
string AllLIFNeurons::toString(const int i) const
{
    stringstream ss;
 ...
 */
void AllLIFNeurons::readNeurons(istream &input, const SimulationInfo *sim_info)
{
 ...
 *  @param  sim_info    used as a reference to set info for neuronss.
 */
void AllLIFNeurons::writeNeurons(ostream& output, const SimulationInfo *sim_info) const 
{
    for (int i = 0; i < sim_info->totalNeurons; i++) {
        output << Cm[i] << ends;
        output << Rm[i] << ends;
