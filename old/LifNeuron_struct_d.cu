...
void allocNeuronStruct_d( int count ) {
	...
	HANDLE_ERROR( cudaMalloc( ( void ** ) &neuron.summationPoint, count * sizeof( PBGFLOAT ) ) );
	...
	HANDLE_ERROR( cudaMalloc( ( void ** ) &neuron.randNoise, count * sizeof( PBGFLOAT ) ) );
...
void copyNeuronHostToDevice( LifNeuron_struct& neuron_h, int count ) {
	...
	HANDLE_ERROR( cudaMemcpy ( neuron.deltaT, neuron_h.deltaT, count * sizeof( BGFLOAT ), cudaMemcpyHostToDevice ) );
	HANDLE_ERROR( cudaMemcpy ( neuron.summationPoint, neuron_h.summationPoint, count * sizeof( PBGFLOAT ), cudaMemcpyHostToDevice ) );
