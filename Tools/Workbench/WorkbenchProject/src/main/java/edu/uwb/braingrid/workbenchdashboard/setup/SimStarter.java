...
public class SimStarter extends WorkbenchApp {
	...
	private HBox initAttributes() {
		...
		VBox btns = new VBox(configure_sim_btn_, specify_script_btn_, script_gen_btn_, run_script_btn_, analyze_output_btn_);
		VBox lbls = new VBox(simulation_lbl_, script_specify_lbl_, script_generated_lbl_, run_script_lbl_, analyze_output_sts_lbl_);
		VBox txt = new VBox(simulation_out_lbl_, script_specify_out_lbl_, script_generated_out_lbl_, run_script_out_lbl_, analyze_output_sts_out_lbl_);
...
//	    }
 ...
 private void analyzeOutputButtonActionPerformed() {
     ...
     if (timeCompleted != DateTime.ERROR_TIME) {
     	analyze_output_sts_lbl_.setText("Completed at: "
                 + DateTime.getTime(timeCompleted));
     } else {
     	analyze_output_sts_lbl_.setText("Script execution incomplete, try again later.");
     }
     setMsg();
 }
