            ...
            if (timeCompleted != DateTime.ERROR_TIME && provMgr != null) {
                ...
                if (sim != null && simSuccessful) {
                    ...
                    Resource simOutputFile = provMgr.addEntity(remoteOutputFilename,
                            "simOutput", simSpec.getHostAddr(), simSpec.getUsername(),"sftp", false);
                    ...
                    String remoteSimConfigFilename = userDir + simSpec.getSimulatorFolder()
                            + "/workbenchconfigfiles/" + FileManager.getSimpleFilename(projectMgr.getSimConfigFilename());
                    Resource remoteSimConfigFileEnitity = provMgr.addEntity(remoteSimConfigFilename,
                            "simulationConfigurationFile", simSpec.getHostAddr(), simSpec.getUsername(), "sftp", false);
                    ...
                    for (int i = 0, im = neuronLists.length; i < im; i++) {
                        ...
                        Resource movedNLFileEntity = provMgr.addEntity(movedNLFilename,
                                "nlist", simSpec.getHostAddr(), simSpec.getUsername(),"sftp", false);
                                ...
                                if (!sha1key.contains("fatal")) {
                                    provMgr.wasDerivedFrom(simAgent,
                                            provMgr.addEntity(
                                                    ...
                                                    .substring(8, simSpec.getCodeLocation().lastIndexOf("."))
                                                    + "/commit/" + sha1key, "commit", null, null,
                                                    "https", false));
                                }
                            }
