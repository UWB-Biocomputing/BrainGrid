...
public class ControlFrame extends JFrame implements ActionListener {
	 ...
	 * 
	 * @param sizeX
	 *            width of the layout panel.
	 * @param sizeY
	 *            height of the layoujt panel.
	 */
		...
		try {
			...
			if ((root != null)
					&& ((root.getName() == "A" && type == LayoutPanel.ACT)
							|| (root.getName() == "I" && type == LayoutPanel.INH) || (root
	...
	public String getStatisticalMsg(boolean bHtml) {
		...
		float nniInhNList = (float) calcNearestNeighborIndex(cnvIndexList2Points(layoutPanel.inhNList));
		float nniActNList = (float) calcNearestNeighborIndex(cnvIndexList2Points(layoutPanel.activeNList));
		...
		float nniPrbNList = (float) calcNearestNeighborIndex(cnvIndexList2Points(layoutPanel.probedNList));
